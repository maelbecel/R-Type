...
namespace RType {
    ...
    void Tabrok::CreateAnimations() {
        ...
        for (int i = 0; i < 5; i++)
            framesIdle.push_back(SubTexture2D::CreateFromCoords(TABROK, {i, 2.0f}, {49.7f, 57.5}, {1.0f, 1.0f}));
        for (int i = 0; i < 6; i++)
    ...
    void Tabrok::OnCreate() {
        ...
        EXODIA_INFO("Tabrok created at pos {0}, {1}", tc.Translation.x, tc.Translation.y);
    }

    void Tabrok::UpdateAnimations() {
    ...
    void Tabrok::OnUpdate(Timestep ts) {
        IsDead();
        UpdateAnimations();
        Shoot();
    }

    void Tabrok::OnCollisionEnter(Entity *entity) {
        ComponentHandle<TagComponent> tag = entity->GetComponent<TagComponent>();
        ComponentHandle<Health> health = GetComponent<Health>();

        if (!tag || !health) {
            EXODIA_WARN("Tabrok: No tag or health component found");
            return;
        }
    ...
    void Tabrok::IsDead() {
        ...
        if (!health || !body) {
            EXODIA_WARN("Tabrok has no health or body");
            return;
        }
