...
namespace Exodia
{
    ...
    class ButtonScript : public ScriptableEntity
    {
      ...
      public:
        ...
        {
            EXODIA_INFO( "Button Create : Click should be between X : {0} -> {1}, Y {2} -> {3}",
                         GetComponent<TransformComponent>().Get().Translation.x,
                         GetComponent<TransformComponent>().Get().Translation.x +
                             GetComponent<TransformComponent>().Get().Scale.x,
                         GetComponent<TransformComponent>().Get().Translation.y,
                         GetComponent<TransformComponent>().Get().Translation.y +
                             GetComponent<TransformComponent>().Get().Scale.y );
        }
