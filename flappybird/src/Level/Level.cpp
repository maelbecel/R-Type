...
void Level::OnRender()
{
    ...
    Exodia::Renderer2D::DrawQuad(
        { playerPos.x, 0.0f, -0.8f }, // Position
        { 50.0f, 50.0f },             // Size
...
bool Level::Collision()
{
    ...
    for (int i = 0; i < 4; i++)
        playerTransformedVerts[i] = glm::translate(glm::mat4(1.0f), { pos.x, pos.y, 0.0f }) * glm::rotate(glm::mat4(1.0f), glm::radians(_Player.GetRotation()), { 0.0f, 0.0f, 1.0f }) * glm::scale(glm::mat4(1.0f), { 1.0f, 1.3f, 1.0f }) * playerVertices[i];
    ...
    for (Obstacle &obstacle : _Obstacles) {
        ...
        // Top pillars
        for (int i = 0; i < 3; i++)
            triangle[i] = glm::translate(glm::mat4(1.0f), { obstacle.PositionTop.x, obstacle.PositionTop.y, 0.0f }) * glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), { 0.0f, 0.0f, 1.0f }) * glm::scale(glm::mat4(1.0f), { obstacle.TopScale.x, obstacle.TopScale.y, 1.0f }) * pillarVertices[i];
        ...
        // Bottom pillars
        for (int i = 0; i < 3; i++) {
            triangle[i] = glm::translate(glm::mat4(1.0f), { obstacle.PositionBottom.x, obstacle.PositionBottom.y, 0.0f }) * glm::scale(glm::mat4(1.0f), { obstacle.BottomScale.x, obstacle.BottomScale.y, 1.0f }) * pillarVertices[i];
        }
