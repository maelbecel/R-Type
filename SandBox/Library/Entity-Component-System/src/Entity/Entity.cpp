...
namespace Exodia {
    ...
    Entity *Entity::Duplicate(World *world, UUID uuid, const std::string &name) {
        ...
        for (auto pair : _Components)
            entity->_Components[pair.first] = pair.second->Duplicate();
        return entity;
    }

    Entity *Entity::NewDuplicate(World *world, UUID uuid, const std::string &name) {
        Entity *entity = world->CreateNewEntity(uuid, name);
    ...
    bool Entity::Has(const std::string &index) { return _Components.find(index) != _Components.end(); }
