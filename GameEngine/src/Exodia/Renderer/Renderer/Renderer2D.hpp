...
namespace Exodia {
    ...
    /**
     * @brief The Renderer2D class handles 2D rendering operations, including rendering primitives like quads and
     * rotated quads.
     */
    class Renderer2D {
      ...
      public:
         ...
         */
        struct CircleVertex {
            glm::vec3 WorldPosition; /* !< The world position of the vertex. (World position is the position of the
                                        vertex relative to the world) */
            glm::vec3 LocalPosition; /* !< The local position of the vertex. (Local position is the position of the
                                        vertex relative to the center of the circle) */
            glm::vec4 Color;         /* !< The color of the vertex. */
            float Thickness;         /* !< The thickness of the circle. */
            float Fade; /* !< The fade of the circle. (Fade is the distance between the vertex and the center of the
                           circle) */
      ...
      public:
        /**
         * @brief Initialize the Renderer2D class. This function sets up the necessary resources and data structures for
         * rendering.
         */
        static void Init();

        /**
         * @brief Shutdown the Renderer2D class. This function releases any allocated resources and cleans up the
         ...
         * @param tintColor    (Type: glm::vec4)              The tint color to apply to the quad (default is
         * glm::vec4(1.0f)).
         *
         * @overload
         * @param transform    (Type: glm::vec4)              The transform of the quad.
        ...
        static void DrawLine(const glm::vec3 &pointA, const glm::vec3 &pointB, const glm::vec4 &color,
                             int entityID = -1);
        ...
        static void DrawText(const glm::mat4 &transform, const std::string &text, const TextRendererComponent &component, int entityID = -1);
        static void DrawText(const glm::mat4 &transform, const std::string &text, Ref<Font> font, const TextData &params, int entityID = -1);
