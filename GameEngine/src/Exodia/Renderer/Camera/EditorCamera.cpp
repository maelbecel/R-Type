...
namespace Exodia {
    ...
    EditorCamera::EditorCamera(float fov, float aspectRatio, float nearClip, float farClip)
        : _FOV(fov), _AspectRatio(aspectRatio), _NearClip(nearClip), _FarClip(farClip), _Position(glm::vec3(0.0f)),
          _FocalPoint(glm::vec3(0.0f)), _InitialMousePosition(glm::vec2(0.0f)), _Distance(10.0f), _Pitch(0.0f),
          _Yaw(0.0f), _ViewportWidth(1280), _ViewportHeight(720) {
        UpdateView();
    }
    ...
    glm::vec3 EditorCamera::CalculatePosition() const { return _FocalPoint - GetForwardDirection() * _Distance; }

    ///////////////////////
    // Getters & Setters //
    ///////////////////////

    const glm::mat4 &EditorCamera::GetViewMatrix() const { return _ViewMatrix; }

    glm::mat4 EditorCamera::GetViewProjection() const { return _Projection * _ViewMatrix; }

    const glm::vec3 &EditorCamera::GetPosition() const { return _Position; }

    float EditorCamera::GetPitch() const { return _Pitch; }

    float EditorCamera::GetYaw() const { return _Yaw; }
    ...
    glm::quat EditorCamera::GetOrientation() const { return glm::quat(glm::vec3(-_Pitch, -_Yaw, 0.0f)); }
