...
class Connection {
    public:
        ...
        void SetSendPacket(int packet) { _lastNetworkInfo.sendPacket = _networkInfo.sendPacket;_networkInfo.sendPacket = packet;}

        void AddReceivedPacket() { _networkInfo.receivedPacket++; }

        void AddKyloByteReceived(Exodia::Network::Packet packet) { _networkInfo.kiloByteReceived += packet.GetBuffer().size() / 1024.0f; }

        void SetReceivedPacket(int packet) { _lastNetworkInfo.receivedPacket = _networkInfo.receivedPacket;_networkInfo.receivedPacket = packet;}
        ...
        void SetKiloByteSent(float kiloByteSent) { _lastNetworkInfo.kiloByteSent = _networkInfo.kiloByteSent;_networkInfo.kiloByteSent = kiloByteSent; }

        void SetKiloByteReceived(float kiloByteReceived) { _lastNetworkInfo.kiloByteReceived = _networkInfo.kiloByteReceived;_networkInfo.kiloByteReceived = kiloByteReceived; }

        void SetPing(uint16_t ping) { _lastNetworkInfo.ping = _networkInfo.ping;_networkInfo.ping = ping; }
